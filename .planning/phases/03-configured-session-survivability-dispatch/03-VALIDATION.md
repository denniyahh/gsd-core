...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 03-01-01 | 01 | 1 | — | — | Establish the current upstream/next implementation baseline without overwriting local work. | Preflight | `git fetch upstream && git merge-base --is-ancestor upstream/next HEAD && git diff --check && npm run check:env` | ✅ | ✅ green |
...
| 03-02-01 | 02 | 2 | SESSION-03 | — | ADR and generated index remain synchronized. | Generator | `node scripts/gen-adr-index.cjs --check && npm run lint:generated-sync` | ✅ | ✅ green |
| 03-02-02 | 02 | 2 | QUALITY-03, COMPAT-01 | T-03-02, T-03-03 | Runtime conversion and install projections preserve both dispatch directions. | Projection and install integration | `npm run build:lib && node --test tests/runtime-converters.test.cjs tests/adr-index-gate.test.cjs && npm run test:install && npm run lint:generated-sync` | ✅ | ✅ green |
| 03-02-03 | 02 | 2 | — | — | Complete post-verification ship handoff only after a real PR exists and the generated changeset passes lint. | Ship handoff | `GITHUB_BASE_REF=next node scripts/changeset/lint.cjs` | ✅ | ✅ green |
